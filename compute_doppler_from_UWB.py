...
for ss, act in zip(indices, acts):
	...
	hp = sns.heatmap(range_profile_seg)
	plt.axvline(x=left, color='r')
	plt.axvline(x=right, color='r')
	plt.title("range profile of activity {}".format(activity))
	plt.ylabel("time (1/{} second)".format(fps_uwb))
	plt.xlabel("range bin")
	hp.figure.savefig(os.path.join(path, "range_profile_{}_{}.png".format(activity, node_num)))
	plt.show()
	...
	plt.title("doppler of activity {}".format(activity))
	plt.ylabel("doppler")
	plt.xlabel("time")
	hp.figure.savefig(os.path.join(path, "doppler_{}_{}.png".format(activity, node_num)))
	plt.show()
